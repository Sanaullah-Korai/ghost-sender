...
<label>From (spoofed)</label><input name="from" value="security@alrayan.com">
<label>To (comma-separated)</label><input name="to" value="OISSG@alrayan.com"><label>Cc (comma-separated)</label><input name="cc" placeholder="user2@alrayan.com, user3@alrayan.com">
...
{''.join(f'<div class="card"><div class="red">{f["to"]}: {f.get("error","?")[:200]}</div></div>' for f in server.failed[-10:])}
    ...
    def do_POST(self):
        if self.path == '/send':
            ...
            from_addr = params.get('from', 'security@alrayan.com')
            to_raw = params.get('to', 'OISSG@alrayan.com')
